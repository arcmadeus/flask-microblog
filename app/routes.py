...
def index():
	...
	The template receives the form object as an additional argument, so that it can render it to the page.
	"""
	form = PostForm()
	if form.validate_on_submit():
		...
		return redirect(url_for('index')) # a standard practice to always respond to a POST request generated by a web form submission with a redirect to counter REFRESH's duplication of the form data by giving a GET request; (POST/REDIRECT/GET) pattern.
	
	# Getting the posts of the followed users.
...
def explore():
	"""
	Selects ALL posts from the database and orders them by their timestamp, even if the user does not follow any users.
	"""
	page = request.args.get('page', 1, type=int)
